...
### stacking decorators for layered permissions ###

import functools

# CCTV — records everything, even the bouncer's decisions
def logger(func):
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        print(f"CCTV: '{func.__name__}' session started")
        result = func(*args, **kwargs)
        print(f"CCTV: '{func.__name__}' session ended")
        return result
    return wrapper

# Bouncer — checks ID before letting anyone into the club
def requires_permission(permission_level: str):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(user_role: str, *args, **kwargs):
            if user_role == permission_level or user_role == "admin":
                print(f"BOUNCER: '{user_role}' is on the list, enter!")
                return func(user_role, *args, **kwargs)
            else:
                print(f"BOUNCER: '{user_role}' not on the list, go home!")
                raise PermissionError(f"No entry for '{user_role}'")
        return wrapper
    return decorator

# CCTV outside, Bouncer inside — top to bottom
@logger
@requires_permission("data_scientist")
def deploy_model(user_role, model_id, environment):
    # DJ doing his job
    print(f"DJ '{user_role}' dropping '{model_id}' in '{environment}'")
